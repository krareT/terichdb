...
class TERARK_DB_DLL DB {
 ...
 static Status ListColumnFamilies(const Options& db_options,
  ...
  virtual void GetApproximateSizes(const Range*, int n, uint64_t* sizes) = 0;
  ...
  // the timestamp.
  virtual Status GetReplayIterator(const std::string& timestamp, ReplayIterator**) = 0;
