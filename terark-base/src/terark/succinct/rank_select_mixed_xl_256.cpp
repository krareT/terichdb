...
rank_select_mixed_xl_256::rank_select_mixed_xl_256() {
    m_lines = NULL;
    m_capacity = 0;
    m_size[0] = m_size[1] = 0;
    nullize_cache();
...
rank_select_mixed_xl_256::rank_select_mixed_xl_256(size_t n, bool val0, bool val1) {
...
rank_select_mixed_xl_256::rank_select_mixed_xl_256(size_t n, valvec_no_init) {
    rank_select_check_overflow(n, > , rank_select_mixed_xl_256);
    m_capacity = BitsToLines(n) * sizeof(RankCacheMixed);
    m_size[0] = m_size[1] = n;
    m_lines = (RankCacheMixed*)malloc(m_capacity);
...
rank_select_mixed_xl_256::rank_select_mixed_xl_256(size_t n, valvec_reserve) {
    rank_select_check_overflow(n, > , rank_select_mixed_xl_256);
    m_capacity = BitsToLines(n) * sizeof(RankCacheMixed);
    m_size[0] = m_size[1] = 0;
    m_lines = (RankCacheMixed*)malloc(m_capacity);
...
rank_select_mixed_xl_256::rank_select_mixed_xl_256(const rank_select_mixed_xl_256& y)
...
rank_select_mixed_xl_256& rank_select_mixed_xl_256::operator=(const rank_select_mixed_xl_256& y) {
    if (this != &y) {
        this->clear();
...
rank_select_mixed_xl_256::rank_select_mixed_xl_256(rank_select_mixed_xl_256&& y) noexcept {
    memcpy(this, &y, sizeof(*this));
    y.risk_release_ownership();
}

rank_select_mixed_xl_256& rank_select_mixed_xl_256::operator=(rank_select_mixed_xl_256&& y) noexcept {
    if (m_lines)
        ::free(m_lines);
    memcpy(this, &y, sizeof(*this));
    y.risk_release_ownership();
...
void rank_select_mixed_xl_256::clear() {
    if (m_lines)
        ::free(m_lines);
    risk_release_ownership();
}

void rank_select_mixed_xl_256::risk_release_ownership() {
    nullize_cache();
    m_lines = nullptr;
    m_capacity = 0;
    m_size[0] = m_size[1] = 0;
...
void rank_select_mixed_xl_256::risk_mmap_from(unsigned char* base, size_t length) {
    assert(NULL == m_lines);
...
void rank_select_mixed_xl_256::shrink_to_fit() {
...
void rank_select_mixed_xl_256::swap(rank_select_mixed_xl_256& y) {
...
const void* rank_select_mixed_xl_256::data() const {
    return m_lines;
}

size_t rank_select_mixed_xl_256::mem_size() const {
    return m_capacity;
}

void rank_select_mixed_xl_256::grow() {
...
void rank_select_mixed_xl_256::reserve_bytes(size_t capacity) {
...
void rank_select_mixed_xl_256::bits_range_set0_dx(size_t i, size_t k) {
    if (i == k) {
        return;
    }
    ...
    size_t j = i / UintBits;
    if (j == (k - 1) / UintBits) {
        m_lines[j / LineWords].words[(j % LineWords) * 2 + dimensions] &= ~(bm_uint_t(-1) << i % UintBits)
        ...
        while (j < k / UintBits) {
            m_lines[j / LineWords].words[(j % LineWords) * 2 + dimensions] = 0;
            ++j;
        }
        if (k % UintBits)
            m_lines[j / LineWords].words[(j % LineWords) * 2 + dimensions] &= bm_uint_t(-1) << k % UintBits;
    }
}

template void TERARK_DLL_EXPORT rank_select_mixed_xl_256::bits_range_set0_dx<0>(size_t i, size_t k);
template void TERARK_DLL_EXPORT rank_select_mixed_xl_256::bits_range_set0_dx<1>(size_t i, size_t k);

template<size_t dimensions>
void rank_select_mixed_xl_256::bits_range_set1_dx(size_t i, size_t k) {
    if (i == k) {
        return;
    }
    const static size_t UintBits = sizeof(bm_uint_t) * 8;
    ...
    if (j == (k - 1) / UintBits) {
        ...
        while (j < k / UintBits) {
            m_lines[j / LineWords].words[(j % LineWords) * 2 + dimensions] = bm_uint_t(-1);
            ++j;
        }
        if (k % UintBits)
            m_lines[j / LineWords].words[(j % LineWords) * 2 + dimensions] |= ~(bm_uint_t(-1) << k % UintBits);
...
template void TERARK_DLL_EXPORT rank_select_mixed_xl_256::bits_range_set1_dx<0>(size_t i, size_t k);
template void TERARK_DLL_EXPORT rank_select_mixed_xl_256::bits_range_set1_dx<1>(size_t i, size_t k);

template<size_t dimensions>
void rank_select_mixed_xl_256::build_cache_dx(bool speed_select0, bool speed_select1) {
    ...
    for(size_t i = 0; i < lines; ++i) {
        ...
        for (size_t j = 0; j < 4; ++j) {
            ...
            inc += fast_popcount(m_lines[i].bit64[j * 2 + dimensions]);
        }
        Rank1 += inc;
    }
...
template void TERARK_DLL_EXPORT rank_select_mixed_xl_256::build_cache_dx<0>(bool speed_select0, bool speed_select1);
template void TERARK_DLL_EXPORT rank_select_mixed_xl_256::build_cache_dx<1>(bool speed_select0, bool speed_select1);

template<size_t dimensions>
size_t rank_select_mixed_xl_256::one_seq_len_dx(size_t bitpos) const {
    assert(bitpos < m_size[dimensions]);
    size_t j = bitpos / LineBits, k, sum;
    if (bitpos % WordBits != 0) {
        bm_uint_t x = m_lines[j].words[(bitpos % LineBits / WordBits) * 2 + dimensions];
        ...
        k = (bitpos % LineBits / WordBits) * 2 + dimensions;
        sum = 0;
    }
    size_t len = BitsToLines(m_size[dimensions]) + 1;
    for (; j < len; ++j) {
        for (; k < LineWords * 2; k += 2) {
...
template size_t TERARK_DLL_EXPORT rank_select_mixed_xl_256::one_seq_len_dx<0>(size_t bitpos) const;
template size_t TERARK_DLL_EXPORT rank_select_mixed_xl_256::one_seq_len_dx<1>(size_t bitpos) const;

template<size_t dimensions>
size_t rank_select_mixed_xl_256::zero_seq_len_dx(size_t bitpos) const {
    assert(bitpos < m_size[dimensions]);
    size_t j = bitpos / LineBits, k, sum;
    if (bitpos % WordBits != 0) {
        bm_uint_t x = m_lines[j].words[(bitpos % LineBits / WordBits) * 2 + dimensions];
...
template size_t TERARK_DLL_EXPORT rank_select_mixed_xl_256::select0_dx<0>(size_t Rank0) const;
template size_t TERARK_DLL_EXPORT rank_select_mixed_xl_256::select0_dx<1>(size_t Rank0) const;

template<size_t dimensions>
size_t rank_select_mixed_xl_256::select1_dx(size_t Rank1) const {
    assert(m_flags & (1 << (dimensions == 0 ? 1 : 4)));
...
template size_t TERARK_DLL_EXPORT rank_select_mixed_xl_256::select1_dx<0>(size_t Rank1) const;
template size_t TERARK_DLL_EXPORT rank_select_mixed_xl_256::select1_dx<1>(size_t Rank1) const;

template class TERARK_DLL_EXPORT rank_select_mixed_dimensions<rank_select_mixed_xl_256, 0>;
template class TERARK_DLL_EXPORT rank_select_mixed_dimensions<rank_select_mixed_xl_256, 1>;
