...
//#define RS_ITERATOR_TRACE(x) log() << "TerichDbRS::Iterator " << x
...
//MONGO_FP_DECLARE(TerichDbWriteConflictException);

class TerichDbRecordStore::Cursor final : public SeekableRecordCursor, public ICleanOnOwnerDead {
public:
    Cursor(OperationContext* txn, const TerichDbRecordStore& rs, bool forward)
        : _rs(rs),
          _txn(txn), _forward(forward) {
		LOG(1) << "TerichDbRecordStore::Cursor::Cursor(): forward = " << forward;
		init(txn);
	...
	~Cursor() {
		LOG(1) << "TerichDbRecordStore::Cursor::~Cursor(): forward = " << _forward
    ...
    boost::optional<Record> next() final {
        ...
        if (!_skipNextAdvance) {
			...
			if (_forward && _lastReturnedId.repr() >= recIdx+1) {
				LOG(1) << "TerichDbRecordStore::Cursor::next -- c->next_key ( " << RecordId(recIdx+1)
		...
		LOG(1) << "TerichDbRecordStore::Cursor::next(): _skipNextAdvance = " << _skipNextAdvance
			   << ", RecordBson(" << id << "): "
    ...
    boost::optional<Record> seekExact(const RecordId& id) final {
		LOG(1) << "TerichDbRecordStore::Cursor::seekExact(): id = " << id
    ...
    void save() final {
		LOG(1) << "TerichDbRecordStore::Cursor::save(): _skipNextAdvance = " << _skipNextAdvance
			<< ", _eof = " << _eof << ", _lastReturnedId = " << _lastReturnedId;
		do_save();
	}
    ...
    void saveUnpositioned() final {
		LOG(1) << "TerichDbRecordStore::Cursor::saveUnpositioned(): _skipNextAdvance = " << _skipNextAdvance
			<< ", _eof = " << _eof << ", _lastReturnedId = " << _lastReturnedId;
        do_save();
    ...
    bool restore() override final {
        ...
        if (_eof) {
			LOG(1) << "TerichDbRecordStore::Cursor::restore(): _skipNextAdvance = " << _skipNextAdvance
				<< ", _eof = " << _eof << ", _lastReturnedId = " << _lastReturnedId;
	        _skipNextAdvance = false;
            return true;
		}
    ...
    void detachFromOperationContext() final {
		LOG(1) << "TerichDbRecordStore::Cursor::detachFromOperationContext(): _skipNextAdvance = " << _skipNextAdvance
			<< ", _eof = " << _eof << ", _lastReturnedId = " << _lastReturnedId;
    ...
    void reattachToOperationContext(OperationContext* txn) final {
		LOG(1) << "TerichDbRecordStore::Cursor::reattachToOperationContext(): _skipNextAdvance = " << _skipNextAdvance
			<< ", _eof = " << _eof << ", _lastReturnedId = " << _lastReturnedId;
        _txn = txn;
		init(txn);
    }
...
private:
    ...
    terark::terichdb::StoreIteratorPtr _cursor;
...
StatusWith<std::string> parseOptionsField(const BSONObj options) {
    ...
    BSONForEach(elem, options) {
        ...
        /*    Status status = TerichDbUtil::checkTableCreationOptions(elem);
            if (!status.isOK()) {
                return status;
            }*/
...
TerichDbRecordStore::TerichDbRecordStore(OperationContext* ctx,
									 StringData ns,
									 StringData ident,
									 ThreadSafeTable* tab,
									 TerichDbSizeStorer* sizeStorer)
		: RecordStore(ns),
		  m_table(tab),
...
TerichDbRecordStore::~TerichDbRecordStore() {
    _shuttingDown = true;
...
const char* TerichDbRecordStore::name() const {
    return kTerichDbEngineName.c_str();
}

bool TerichDbRecordStore::inShutdown() const {
    return _shuttingDown;
}

long long TerichDbRecordStore::dataSize(OperationContext* txn) const {
    return m_table->m_tab->dataStorageSize();
}

long long TerichDbRecordStore::numRecords(OperationContext* txn) const {
	auto tab = m_table->m_tab.get();
	long long existing = tab->existingRows();
	long long total = tab->inlineGetRowNum();
    LOG(1) << "TerichDbRecordStore::numRecords(): existing = " << existing
		<< ", total" << total << ", dir: " << tab->getDir().string();
    return existing;
}

bool TerichDbRecordStore::isCapped() const {
    return false;
}

int64_t TerichDbRecordStore::storageSize(OperationContext* txn,
...
RecordData
TerichDbRecordStore::dataFor(OperationContext* txn, const RecordId& id) const {
	return RecordStore::dataFor(txn, id);
}

bool TerichDbRecordStore::findRecord(OperationContext* txn,
								   const RecordId& id,
								   RecordData* out) const {
	DbTable* tab = m_table->m_tab.get();
	if (id.isNull()) {
		LOG(2) << "TerichDbRecordStore::findRecord(): id = null, dir: " << tab->getDir().string();
		return false;
	}
    llong recIdx = id.repr() - 1;
	...
	LOG(2) << "TerichDbRecordStore::findRecord(): id = " << id
		<< ", rud = " << (void*)rud.get()
...
void TerichDbRecordStore::deleteRecord(OperationContext* txn, const RecordId& id) {
    auto& td = m_table->getMyThreadData();
	auto tab = m_table->m_tab.get();
	if (txn && txn->recoveryUnit()) {
		LOG(2) << "TerichDbRecordStore::deleteRecord(): id = " << id
...
Status TerichDbRecordStore::insertRecords(OperationContext* txn,
										std::vector<Record>* records,
										bool enforceQuota) {
	DbTable* tab = m_table->m_tab.get();
    auto& td = m_table->getMyThreadData();
	if (0 == records->size()) {
	    LOG(1) << "TerichDbRecordStore::insertRecords(): records->size() = 0";
	}
    for (size_t i = 0; i < records->size(); ++i) {
	    ...
	    LOG(2) << "TerichDbRecordStore::insertRecords(): i = " << i
			<< ", bson = " << bson.toString() << ", id = " << rec.id;
    }
    return Status::OK();
}

StatusWith<RecordId> TerichDbRecordStore::insertRecord(OperationContext* txn,
													 const char* data,
													 int len,
													 bool enforceQuota) {
	DbTable* tab = m_table->m_tab.get();
    ...
    LOG(2) << "TerichDbRecordStore::insertRecord(): bson = " << bson.toString();
    try {
		td.m_coder.encode(&tab->rowSchema(), nullptr, bson, &td.m_buf);
	} catch (const std::exception& ex) {
		return Status(ErrorCodes::InvalidBSON, ex.what());
...
Status
TerichDbRecordStore::insertRecordsWithDocWriter(OperationContext* txn,
                                            const DocWriter* const* docs,
                                            size_t nDocs,
                                            RecordId* idsOut) {
	if (0 == nDocs) {
	    LOG(1) << "TerichDbRecordStore::insertRecordsWithDocWriter(): nDocs = 0";
		return Status::OK();
	}
	DbTable* tab = m_table->m_tab.get();
    auto& td = m_table->getMyThreadData();
	...
	terark::terichdb::BatchWriter batch(tab, td.m_dbCtx.get());
    for (size_t i = 0; i < nDocs; i++) {
        docs[i]->writeDocument(pos);
		...
		try {
			...
			LOG(2) << "TerichDbRecordStore::insertRecordsWithDocWriter(): i = " << i
	...
	if (!batch.commit()) {
		return Status(ErrorCodes::OperationFailed, "TerichDbRecordStore::insertRecordsWithDocWriter: terark::terichdb::BatchWriter::commit failed");
	}
...
Status
TerichDbRecordStore::updateRecord(OperationContext* txn,
								const RecordId& id,
								const char* data,
								int len,
								bool enforceQuota,
	...
	DbTable* tab = m_table->m_tab.get();
	terark::terichdb::IncrementGuard_size_t incrGuard(tab->m_inprogressWritingCount);
	invariant(id.repr() != 0);
	llong recId = id.repr() - 1;
    BSONObj bson(data);
	{
		terark::terichdb::MyRwLock lock(tab->m_rwMutex, false);
		size_t segIdx = tab->getSegmentIndexOfRecordIdNoLock(recId);
		if (segIdx >= tab->getSegNum()) {
			LOG(2) << "TerichDbRecordStore::updateRecord(): bson = " << bson.toString()
		...
		if (txn && txn->recoveryUnit()) {
			...
			return {ErrorCodes::NeedsDocumentMove, "TerichDB is in recovery unit, needs move"};
		}
	}
	LOG(2) << "TerichDbRecordStore::updateRecord(): bson = " << bson.toString()
...
bool TerichDbRecordStore::updateWithDamagesSupported() const {
    return false;
}

StatusWith<RecordData> TerichDbRecordStore::updateWithDamages(
...
TerichDbRecordStore::getCursor(OperationContext* txn, bool forward) const {
    return stdx::make_unique<Cursor>(txn, *this, forward);
}

std::unique_ptr<RecordCursor>
TerichDbRecordStore::getRandomCursor(OperationContext* txn) const {
    return nullptr;
}

std::vector<std::unique_ptr<RecordCursor>>
TerichDbRecordStore::getManyCursors(OperationContext* txn) const {
    std::vector<std::unique_ptr<RecordCursor>> cursors(1);
    cursors[0] = stdx::make_unique<Cursor>(txn, *this, /*forward=*/true);
    return cursors;
}

Status TerichDbRecordStore::truncate(OperationContext* txn) {
	DbTable* tab = m_table->m_tab.get();
	LOG(2) << "TerichDbRecordStore::truncate()";
	tab->clear();
    return Status::OK();
}

Status TerichDbRecordStore::compact(OperationContext* txn,
...
Status TerichDbRecordStore::validate(OperationContext* txn,
...
void TerichDbRecordStore::appendCustomStats(OperationContext* txn,
										  BSONObjBuilder* result,
										  double scale) const {
    result->appendBool("capped", false);
}

Status TerichDbRecordStore::touch(OperationContext* txn, BSONObjBuilder* output) const {
    return Status::OK();
}

void TerichDbRecordStore::updateStatsAfterRepair(OperationContext* txn,
											   long long numRecords,
											   long long dataSize) {
	LOG(2) << BOOST_CURRENT_FUNCTION << ": is in TODO list, not implemented now";
}

void TerichDbRecordStore::temp_cappedTruncateAfter(OperationContext* txn,
												 RecordId end,
												 bool inclusive) {
	LOG(2) << BOOST_CURRENT_FUNCTION << ": is in TODO list, not implemented now";
}
