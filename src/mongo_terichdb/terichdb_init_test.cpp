...
class TerichDbFactoryTest : public mongo::unittest::Test {
private:
    virtual void setUp() {
        ServiceContext* globalEnv = getGlobalServiceContext();
        ASSERT_TRUE(globalEnv);
        ASSERT_TRUE(getGlobalServiceContext()->isRegisteredStorageEngine(kTerichDbEngineName));
        ...
        while (sfi->more()) {
            ...
            if (currentFactory->getCanonicalName() == kTerichDbEngineName) {
                found = true;
        ...
        ASSERT_TRUE(found);
        _oldOptions = terichDbGlobalOptions;
    }

    virtual void tearDown() {
        terichDbGlobalOptions = _oldOptions;
        factory = NULL;
    }
...
void _testValidateMetadata(const StorageEngine::Factory* factory,
    ...
    terichDbGlobalOptions.directoryForIndexes = directoryForIndexes;
...
TEST_F(TerichDbFactoryTest, ValidateMetadataEmptyOptions) {
...
TEST_F(TerichDbFactoryTest, ValidateMetadataDirectoryPerDB) {
...
TEST_F(TerichDbFactoryTest, ValidateMetadataDirectoryForIndexes) {
    _testValidateMetadata(
...
TEST_F(TerichDbFactoryTest, CreateMetadataOptions) {
