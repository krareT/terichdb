...
class TerichDbSnapshotManager final : public SnapshotManager {
    MONGO_DISALLOW_COPYING(TerichDbSnapshotManager);

public:
    explicit TerichDbSnapshotManager(TerichDb_CONNECTION* conn) {
        invariantTerichDbOK(conn->open_session(conn, NULL, NULL, &_session));
    }

    ~TerichDbSnapshotManager() {
        shutdown();
    }
    ...
    //
    // TerichDb-specific methods
    //

    /**
     * Prepares for a shutdown of the TerichDb_CONNECTION.
     */
    void shutdown();
     ...
     */
    SnapshotName beginTransactionOnCommittedSnapshot(TerichDb_SESSION* session) const;
...
private:
    ...
    TerichDb_SESSION* _session;  // only used for dropping snapshots.
};
