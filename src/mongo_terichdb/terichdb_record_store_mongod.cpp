...
class TerichDbRecordStoreThread : public BackgroundJob {
public:
    TerichDbRecordStoreThread(const NamespaceString& ns)
        : BackgroundJob(true /* deleteSelf */), _ns(ns) {
        _name = std::string("TerichDb RecordStoreThread: ") + _ns.toString();
    }
    ...
    bool _deleteExcessDocuments() {
        ...
        checked_cast<TerichDbRecoveryUnit*>(txn.recoveryUnit())->markNoTicketRequired();
        ...
        try {
            ...
            TerichDbRecordStore* rs =
                checked_cast<TerichDbRecordStore*>(collection->getRecordStore());
            ...
            severe() << "error in TerichDbRecordStoreThread: " << e.what();
            fassertFailedNoTrace(!"error in TerichDbRecordStoreThread");
        } catch (...) {
            fassertFailedNoTrace(!"unknown error in TerichDbRecordStoreThread");
        }
        return true;
    }
...
// static
bool TerichDbKVEngine::initRsOplogBackgroundThread(StringData ns) {
    if (!NamespaceString::oplog(ns)) {
        return false;
    }

    if (storageGlobalParams.repair) {
        LOG(1) << "not starting TerichDbRecordStoreThread for " << ns
    ...
    if (_backgroundThreadNamespaces.count(nss)) {
        log() << "TerichDbRecordStoreThread " << ns << " already started";
    } else {
        log() << "Starting TerichDbRecordStoreThread " << ns;
        BackgroundJob* backgroundThread = new TerichDbRecordStoreThread(nss);
        backgroundThread->go();
