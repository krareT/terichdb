...
using terark::terichdb::ColumnType;
...
using terark::terichdb::ColumnMeta;
static
...
void SchemaRecordCoder::encode(const Schema* schema, const Schema* exclude,
	...
	for(size_t i = 0; i < schemaColumn; ++i) {
		...
		case mongo::Bool:
			encoded->push_back(value[0] ? 1 : 0);
			assert(colmeta.type == terark::terichdb::ColumnType::Uint08);
			break;
		case NumberInt:
		...
		//	log() << "encode: OID=" << toHexLower(value, OID::kOIDSize);
			encoded->append(value, OID::kOIDSize);
			assert(colmeta.type == terark::terichdb::ColumnType::Fixed);
		...
		//	log() << "encode: strlen+1=" << elem.valuestrsize() << ", str=" << elem.valuestr();
			if (colmeta.type == terark::terichdb::ColumnType::StrZero) {
		...
		//	assert(0); // deprecated, should not in data
			...
			break;
		case mongo::Array:
			assert(colmeta.type == terark::terichdb::ColumnType::CarBin);
			{
		...
		case CodeWScope:
			...
			break;
		case BinData:
			if (colmeta.type == terark::terichdb::ColumnType::CarBin) {
			...
			else if (colmeta.type == terark::terichdb::ColumnType::StrZero) {
		...
		case RegEx:
			...
			assert(colmeta.type == terark::terichdb::ColumnType::TwoStrZero);
			break;
		default:
			{
				StringBuilder ss;
...
SchemaRecordCoder::decode(const Schema* schema, const char* data, size_t size) {
	assert(nullptr != schema);
	LOG(3) << "SchemaRecordCoder::decode: data=" << schema->toJsonStr(fstring(data, size));
	MyBsonBuilder bb;
	const char* pos = data;
#if 0
	...
	for(size_t i = 0; i < colnum; ++i) {
		...
		case BinData:
			if (colmeta.type == ColumnType::CarBin) {
				...
				THROW_STD(invalid_argument,
					"mongo::BinData must be terichdb CarBin or StrZero");
			}
			break;
		case RegEx:
			{
