// terichdb_record_store_test.cpp
...
class TerichDbHarnessHelper final : public HarnessHelper {
public:
    static TerichDb_CONNECTION* createConnection(StringData dbpath, StringData extraStrings) {
        TerichDb_CONNECTION* conn = NULL;
        ...
        int ret = terichdb_open(dbpath.toString().c_str(), NULL, config.c_str(), &conn);
    ...
    std::unique_ptr<RecordStore> newNonCappedRecordStore(const std::string& ns) {
        TerichDbRecoveryUnit* ru = new TerichDbRecoveryUnit(_sessionCache);
        OperationContextNoop txn(ru);
        string uri = "table:" + ns;

        StatusWith<std::string> result = TerichDbRecordStore::generateCreateString(
            kTerichDbEngineName, ns, CollectionOptions(), "");
        ASSERT_TRUE(result.isOK());
        std::string config = result.getValue();

        {
            WriteUnitOfWork uow(&txn);
            TerichDb_SESSION* s = ru->getSession(&txn)->getSession();
            invariantTerichDbOK(s->create(s, uri.c_str(), config.c_str()));
            uow.commit();
        }

        return stdx::make_unique<TerichDbRecordStore>(
            &txn, ns, uri, kTerichDbEngineName, false, false);
    }
    ...
    std::unique_ptr<RecordStore> newCappedRecordStore(const std::string& ns,
        ...
        StatusWith<std::string> result =
            TerichDbRecordStore::generateCreateString(kTerichDbEngineName, ns, options, "");
        ASSERT_TRUE(result.isOK());
        std::string config = result.getValue();
        ...
        return stdx::make_unique<TerichDbRecordStore>(
            &txn, ns, uri, kTerichDbEngineName, true, false, cappedMaxSize, cappedMaxDocs);
    }

    RecoveryUnit* newRecoveryUnit() final {
        return new TerichDbRecoveryUnit(_sessionCache);
    }
    ...
    TerichDb_CONNECTION* conn() const {
        return _conn;
    }
...
std::unique_ptr<HarnessHelper> newHarnessHelper() {
    return stdx::make_unique<TerichDbHarnessHelper>();
}

TEST(TerichDbRecordStoreTest, GenerateCreateStringEmptyDocument) {
    BSONObj spec = fromjson("{}");
    StatusWith<std::string> result = TerichDbRecordStore::parseOptionsField(spec);
    ASSERT_OK(result.getStatus());
    ASSERT_EQ(result.getValue(), "");  // "," would also be valid.
}

TEST(TerichDbRecordStoreTest, GenerateCreateStringUnknownField) {
...
TEST(TerichDbRecordStoreTest, GenerateCreateStringNonStringConfig) {
...
TEST(TerichDbRecordStoreTest, GenerateCreateStringEmptyConfigString) {
...
TEST(TerichDbRecordStoreTest, GenerateCreateStringInvalidConfigStringOption) {
    BSONObj spec = fromjson("{configString: 'abc=def'}");
    ASSERT_EQ(TerichDbRecordStore::parseOptionsField(spec), ErrorCodes::BadValue);
}

TEST(TerichDbRecordStoreTest, GenerateCreateStringValidConfigStringOption) {
    BSONObj spec = fromjson("{configString: 'prefix_compression=true'}");
    ASSERT_EQ(TerichDbRecordStore::parseOptionsField(spec),
              std::string("prefix_compression=true,"));
}

TEST(TerichDbRecordStoreTest, Isolation1) {
...
TEST(TerichDbRecordStoreTest, Isolation2) {
    unique_ptr<HarnessHelper> harnessHelper(newHarnessHelper());
    unique_ptr<RecordStore> rs(harnessHelper->newNonCappedRecordStore());
...
TEST(TerichDbRecordStoreTest, SizeStorer1) {
    unique_ptr<TerichDbHarnessHelper> harnessHelper(new TerichDbHarnessHelper());
    unique_ptr<RecordStore> rs(harnessHelper->newNonCappedRecordStore());

    string uri = checked_cast<TerichDbRecordStore*>(rs.get())->getURI();

    string indexUri = "table:myindex";
    TerichDbSizeStorer ss(harnessHelper->conn(), indexUri);
    checked_cast<TerichDbRecordStore*>(rs.get())->setSizeStorer(&ss);
    ...
    {
        unique_ptr<OperationContext> opCtx(harnessHelper->newOperationContext());
        rs.reset(new TerichDbRecordStore(
            opCtx.get(), "a.b", uri, kTerichDbEngineName, false, false, -1, -1, NULL, &ss));
    }
    ...
    {
        unique_ptr<OperationContext> opCtx(harnessHelper->newOperationContext());
        TerichDbRecoveryUnit* ru = checked_cast<TerichDbRecoveryUnit*>(opCtx->recoveryUnit());

        {
            WriteUnitOfWork uow(opCtx.get());
            TerichDb_SESSION* s = ru->getSession(opCtx.get())->getSession();
            invariantTerichDbOK(s->create(s, indexUri.c_str(), ""));
            uow.commit();
        }
    ...
    {
        unique_ptr<OperationContext> opCtx(harnessHelper->newOperationContext());
        TerichDbSizeStorer ss2(harnessHelper->conn(), indexUri);
        ss2.fillCache();
        long long numRecords;
        long long dataSize;
...
private:
    virtual void setUp() {
        harnessHelper.reset(new TerichDbHarnessHelper());
        sizeStorer.reset(new TerichDbSizeStorer(harnessHelper->conn(), "table:sizeStorer"));
        rs = harnessHelper->newNonCappedRecordStore();
        TerichDbRecordStore* wtrs = checked_cast<TerichDbRecordStore*>(rs.get());
...
protected:
    ...
    std::unique_ptr<TerichDbHarnessHelper> harnessHelper;
    std::unique_ptr<TerichDbSizeStorer> sizeStorer;
...
TEST_F(SizeStorerValidateTest, InvalidSizeStorerAtCreation) {
    ...
    rs.reset(new TerichDbRecordStore(opCtx.get(),
                                       "a.b",
                                       uri,
                                       kTerichDbEngineName,
                                       false,
...
StatusWith<RecordId> insertBSON(unique_ptr<OperationContext>& opCtx,
    ...
    TerichDbRecordStore* wrs = checked_cast<TerichDbRecordStore*>(rs.get());
    invariant(wrs);
...
// TODO make generic
TEST(TerichDbRecordStoreTest, OplogHack) {
    TerichDbHarnessHelper harnessHelper;
...
TEST(TerichDbRecordStoreTest, OplogHackOnNonOplog) {
    TerichDbHarnessHelper harnessHelper;
...
TEST(TerichDbRecordStoreTest, CappedOrder) {
    unique_ptr<TerichDbHarnessHelper> harnessHelper(new TerichDbHarnessHelper());
...
TEST(TerichDbRecordStoreTest, CappedCursorRollover) {
    unique_ptr<TerichDbHarnessHelper> harnessHelper(new TerichDbHarnessHelper());
...
TEST(TerichDbRecordStoreTest, OplogOrder) {
    unique_ptr<TerichDbHarnessHelper> harnessHelper(new TerichDbHarnessHelper());
    unique_ptr<RecordStore> rs(harnessHelper->newCappedRecordStore("local.oplog.foo", 100000, -1));

    {
        const TerichDbRecordStore* wrs = checked_cast<TerichDbRecordStore*>(rs.get());
        ASSERT(wrs->isOplog());
...
TEST(TerichDbRecordStoreTest, StorageSizeStatisticsDisabled) {
    TerichDbHarnessHelper harnessHelper("statistics=(none)");
...
TEST(TerichDbRecordStoreTest, AppendCustomStatsMetadata) {
    TerichDbHarnessHelper harnessHelper;
    unique_ptr<RecordStore> rs(harnessHelper.newNonCappedRecordStore("a.b"));
    ...
    BSONElement terichDbElement = customStats.getField(kTerichDbEngineName);
    ASSERT_TRUE(terichDbElement.isABSONObj());
    BSONObj terichDb = terichDbElement.Obj();

    BSONElement metadataElement = terichDb.getField("metadata");
    ...
    BSONElement creationStringElement = terichDb.getField("creationString");
    ASSERT_EQUALS(creationStringElement.type(), String);
}

TEST(TerichDbRecordStoreTest, CappedCursorYieldFirst) {
    unique_ptr<TerichDbHarnessHelper> harnessHelper(new TerichDbHarnessHelper());
...
StatusWith<RecordId> insertBSONWithSize(OperationContext* opCtx,
    ...
    TerichDbRecordStore* wtrs = checked_cast<TerichDbRecordStore*>(rs);
    invariant(wtrs);
...
TEST(TerichDbRecordStoreTest, OplogStones_CreateNewStone) {
    TerichDbHarnessHelper harnessHelper;
    ...
    TerichDbRecordStore* wtrs = static_cast<TerichDbRecordStore*>(rs.get());
    TerichDbRecordStore::OplogStones* oplogStones = wtrs->oplogStones();
...
// record is changed.
TEST(TerichDbRecordStoreTest, OplogStones_UpdateRecord) {
    TerichDbHarnessHelper harnessHelper;
...
TEST(TerichDbRecordStoreTest, OplogStones_Truncate) {
    TerichDbHarnessHelper harnessHelper;
...
TEST(TerichDbRecordStoreTest, OplogStones_CappedTruncateAfter) {
    TerichDbHarnessHelper harnessHelper;
...
TEST(TerichDbRecordStoreTest, OplogStones_ReclaimStones) {
    TerichDbHarnessHelper harnessHelper;
...
// 'cappedMaxSize'.
TEST(TerichDbRecordStoreTest, OplogStones_ExceedCappedMaxSize) {
    TerichDbHarnessHelper harnessHelper;
...
TEST(TerichDbRecordStoreTest, OplogStones_AscendingOrder) {
    TerichDbHarnessHelper harnessHelper;
