...
class TerarkDbFactoryTest : public mongo::unittest::Test {
private:
    virtual void setUp() {
        ServiceContext* globalEnv = getGlobalServiceContext();
        ASSERT_TRUE(globalEnv);
        ASSERT_TRUE(getGlobalServiceContext()->isRegisteredStorageEngine(kTerarkDbEngineName));
        ...
        while (sfi->more()) {
            ...
            if (currentFactory->getCanonicalName() == kTerarkDbEngineName) {
                found = true;
        ...
        ASSERT_TRUE(found);
        _oldOptions = terarkDbGlobalOptions;
    }

    virtual void tearDown() {
        terarkDbGlobalOptions = _oldOptions;
        factory = NULL;
    }
...
void _testValidateMetadata(const StorageEngine::Factory* factory,
    ...
    terarkDbGlobalOptions.directoryForIndexes = directoryForIndexes;
...
TEST_F(TerarkDbFactoryTest, ValidateMetadataEmptyOptions) {
...
TEST_F(TerarkDbFactoryTest, ValidateMetadataDirectoryPerDB) {
...
TEST_F(TerarkDbFactoryTest, ValidateMetadataDirectoryForIndexes) {
    _testValidateMetadata(
...
TEST_F(TerarkDbFactoryTest, CreateMetadataOptions) {
