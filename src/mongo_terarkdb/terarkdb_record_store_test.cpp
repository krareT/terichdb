...
class TerarkDbHarnessHelper final : public HarnessHelper {
public:
    static TerarkDb_CONNECTION* createConnection(StringData dbpath, StringData extraStrings) {
        TerarkDb_CONNECTION* conn = NULL;
    ...
    std::unique_ptr<RecordStore> newNonCappedRecordStore(const std::string& ns) {
        TerarkDbRecoveryUnit* ru = new TerarkDbRecoveryUnit(_sessionCache);
        OperationContextNoop txn(ru);
        string uri = "table:" + ns;

        StatusWith<std::string> result = TerarkDbRecordStore::generateCreateString(
            kTerarkDbEngineName, ns, CollectionOptions(), "");
        ASSERT_TRUE(result.isOK());
        std::string config = result.getValue();

        {
            WriteUnitOfWork uow(&txn);
            TerarkDb_SESSION* s = ru->getSession(&txn)->getSession();
            invariantTerarkDbOK(s->create(s, uri.c_str(), config.c_str()));
            uow.commit();
        }

        return stdx::make_unique<TerarkDbRecordStore>(
            &txn, ns, uri, kTerarkDbEngineName, false, false);
    }
    ...
    std::unique_ptr<RecordStore> newCappedRecordStore(const std::string& ns,
        ...
        StatusWith<std::string> result =
            TerarkDbRecordStore::generateCreateString(kTerarkDbEngineName, ns, options, "");
        ASSERT_TRUE(result.isOK());
        std::string config = result.getValue();
        ...
        return stdx::make_unique<TerarkDbRecordStore>(
            &txn, ns, uri, kTerarkDbEngineName, true, false, cappedMaxSize, cappedMaxDocs);
    }

    RecoveryUnit* newRecoveryUnit() final {
        return new TerarkDbRecoveryUnit(_sessionCache);
    }
    ...
    TerarkDb_CONNECTION* conn() const {
        return _conn;
    }
...
std::unique_ptr<HarnessHelper> newHarnessHelper() {
    return stdx::make_unique<TerarkDbHarnessHelper>();
}

TEST(TerarkDbRecordStoreTest, GenerateCreateStringEmptyDocument) {
    BSONObj spec = fromjson("{}");
    StatusWith<std::string> result = TerarkDbRecordStore::parseOptionsField(spec);
    ASSERT_OK(result.getStatus());
    ASSERT_EQ(result.getValue(), "");  // "," would also be valid.
}

TEST(TerarkDbRecordStoreTest, GenerateCreateStringUnknownField) {
...
TEST(TerarkDbRecordStoreTest, GenerateCreateStringNonStringConfig) {
...
TEST(TerarkDbRecordStoreTest, GenerateCreateStringEmptyConfigString) {
...
TEST(TerarkDbRecordStoreTest, GenerateCreateStringInvalidConfigStringOption) {
    BSONObj spec = fromjson("{configString: 'abc=def'}");
    ASSERT_EQ(TerarkDbRecordStore::parseOptionsField(spec), ErrorCodes::BadValue);
}

TEST(TerarkDbRecordStoreTest, GenerateCreateStringValidConfigStringOption) {
    BSONObj spec = fromjson("{configString: 'prefix_compression=true'}");
    ASSERT_EQ(TerarkDbRecordStore::parseOptionsField(spec),
              std::string("prefix_compression=true,"));
}

TEST(TerarkDbRecordStoreTest, Isolation1) {
...
TEST(TerarkDbRecordStoreTest, Isolation2) {
    unique_ptr<HarnessHelper> harnessHelper(newHarnessHelper());
    unique_ptr<RecordStore> rs(harnessHelper->newNonCappedRecordStore());
...
TEST(TerarkDbRecordStoreTest, SizeStorer1) {
    unique_ptr<TerarkDbHarnessHelper> harnessHelper(new TerarkDbHarnessHelper());
    unique_ptr<RecordStore> rs(harnessHelper->newNonCappedRecordStore());

    string uri = checked_cast<TerarkDbRecordStore*>(rs.get())->getURI();

    string indexUri = "table:myindex";
    TerarkDbSizeStorer ss(harnessHelper->conn(), indexUri);
    checked_cast<TerarkDbRecordStore*>(rs.get())->setSizeStorer(&ss);
    ...
    {
        unique_ptr<OperationContext> opCtx(harnessHelper->newOperationContext());
        rs.reset(new TerarkDbRecordStore(
            opCtx.get(), "a.b", uri, kTerarkDbEngineName, false, false, -1, -1, NULL, &ss));
    }
    ...
    {
        unique_ptr<OperationContext> opCtx(harnessHelper->newOperationContext());
        TerarkDbRecoveryUnit* ru = checked_cast<TerarkDbRecoveryUnit*>(opCtx->recoveryUnit());

        {
            WriteUnitOfWork uow(opCtx.get());
            TerarkDb_SESSION* s = ru->getSession(opCtx.get())->getSession();
            invariantTerarkDbOK(s->create(s, indexUri.c_str(), ""));
            uow.commit();
        }
    ...
    {
        unique_ptr<OperationContext> opCtx(harnessHelper->newOperationContext());
        TerarkDbSizeStorer ss2(harnessHelper->conn(), indexUri);
        ss2.fillCache();
        long long numRecords;
        long long dataSize;
...
private:
    virtual void setUp() {
        harnessHelper.reset(new TerarkDbHarnessHelper());
        sizeStorer.reset(new TerarkDbSizeStorer(harnessHelper->conn(), "table:sizeStorer"));
        rs = harnessHelper->newNonCappedRecordStore();
        TerarkDbRecordStore* wtrs = checked_cast<TerarkDbRecordStore*>(rs.get());
...
protected:
    ...
    std::unique_ptr<TerarkDbHarnessHelper> harnessHelper;
    std::unique_ptr<TerarkDbSizeStorer> sizeStorer;
...
TEST_F(SizeStorerValidateTest, InvalidSizeStorerAtCreation) {
    ...
    rs.reset(new TerarkDbRecordStore(opCtx.get(),
                                       "a.b",
                                       uri,
                                       kTerarkDbEngineName,
                                       false,
...
StatusWith<RecordId> insertBSON(unique_ptr<OperationContext>& opCtx,
    ...
    TerarkDbRecordStore* wrs = checked_cast<TerarkDbRecordStore*>(rs.get());
    invariant(wrs);
...
// TODO make generic
TEST(TerarkDbRecordStoreTest, OplogHack) {
    TerarkDbHarnessHelper harnessHelper;
...
TEST(TerarkDbRecordStoreTest, OplogHackOnNonOplog) {
    TerarkDbHarnessHelper harnessHelper;
...
TEST(TerarkDbRecordStoreTest, CappedOrder) {
    unique_ptr<TerarkDbHarnessHelper> harnessHelper(new TerarkDbHarnessHelper());
...
TEST(TerarkDbRecordStoreTest, CappedCursorRollover) {
    unique_ptr<TerarkDbHarnessHelper> harnessHelper(new TerarkDbHarnessHelper());
...
TEST(TerarkDbRecordStoreTest, OplogOrder) {
    unique_ptr<TerarkDbHarnessHelper> harnessHelper(new TerarkDbHarnessHelper());
    unique_ptr<RecordStore> rs(harnessHelper->newCappedRecordStore("local.oplog.foo", 100000, -1));

    {
        const TerarkDbRecordStore* wrs = checked_cast<TerarkDbRecordStore*>(rs.get());
        ASSERT(wrs->isOplog());
...
TEST(TerarkDbRecordStoreTest, StorageSizeStatisticsDisabled) {
    TerarkDbHarnessHelper harnessHelper("statistics=(none)");
...
TEST(TerarkDbRecordStoreTest, AppendCustomStatsMetadata) {
    TerarkDbHarnessHelper harnessHelper;
    unique_ptr<RecordStore> rs(harnessHelper.newNonCappedRecordStore("a.b"));
    ...
    BSONElement terarkDbElement = customStats.getField(kTerarkDbEngineName);
    ASSERT_TRUE(terarkDbElement.isABSONObj());
    BSONObj terarkDb = terarkDbElement.Obj();

    BSONElement metadataElement = terarkDb.getField("metadata");
    ...
    BSONElement creationStringElement = terarkDb.getField("creationString");
    ASSERT_EQUALS(creationStringElement.type(), String);
}

TEST(TerarkDbRecordStoreTest, CappedCursorYieldFirst) {
    unique_ptr<TerarkDbHarnessHelper> harnessHelper(new TerarkDbHarnessHelper());
...
StatusWith<RecordId> insertBSONWithSize(OperationContext* opCtx,
    ...
    TerarkDbRecordStore* wtrs = checked_cast<TerarkDbRecordStore*>(rs);
    invariant(wtrs);
...
TEST(TerarkDbRecordStoreTest, OplogStones_CreateNewStone) {
    TerarkDbHarnessHelper harnessHelper;
    ...
    TerarkDbRecordStore* wtrs = static_cast<TerarkDbRecordStore*>(rs.get());
    TerarkDbRecordStore::OplogStones* oplogStones = wtrs->oplogStones();
...
// record is changed.
TEST(TerarkDbRecordStoreTest, OplogStones_UpdateRecord) {
    TerarkDbHarnessHelper harnessHelper;
...
TEST(TerarkDbRecordStoreTest, OplogStones_Truncate) {
    TerarkDbHarnessHelper harnessHelper;
...
TEST(TerarkDbRecordStoreTest, OplogStones_CappedTruncateAfter) {
    TerarkDbHarnessHelper harnessHelper;
...
TEST(TerarkDbRecordStoreTest, OplogStones_ReclaimStones) {
    TerarkDbHarnessHelper harnessHelper;
...
// 'cappedMaxSize'.
TEST(TerarkDbRecordStoreTest, OplogStones_ExceedCappedMaxSize) {
    TerarkDbHarnessHelper harnessHelper;
...
TEST(TerarkDbRecordStoreTest, OplogStones_AscendingOrder) {
    TerarkDbHarnessHelper harnessHelper;
