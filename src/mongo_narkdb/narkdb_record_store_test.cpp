...
class NarkDbHarnessHelper final : public HarnessHelper {
public:
    static NarkDb_CONNECTION* createConnection(StringData dbpath, StringData extraStrings) {
        NarkDb_CONNECTION* conn = NULL;
    ...
    NarkDbHarnessHelper(StringData extraStrings)
        : _dbpath("wt_test"),
    ...
    std::unique_ptr<RecordStore> newNonCappedRecordStore(const std::string& ns) {
        NarkDbRecoveryUnit* ru = new NarkDbRecoveryUnit(_sessionCache);
        OperationContextNoop txn(ru);
        string uri = "table:" + ns;

        StatusWith<std::string> result = NarkDbRecordStore::generateCreateString(
            kNarkDbEngineName, ns, CollectionOptions(), "");
        ASSERT_TRUE(result.isOK());
        std::string config = result.getValue();

        {
            WriteUnitOfWork uow(&txn);
            NarkDb_SESSION* s = ru->getSession(&txn)->getSession();
            invariantNarkDbOK(s->create(s, uri.c_str(), config.c_str()));
            uow.commit();
        }

        return stdx::make_unique<NarkDbRecordStore>(
            &txn, ns, uri, kNarkDbEngineName, false, false);
    }
    ...
    std::unique_ptr<RecordStore> newCappedRecordStore(const std::string& ns,
        ...
        StatusWith<std::string> result =
            NarkDbRecordStore::generateCreateString(kNarkDbEngineName, ns, options, "");
        ASSERT_TRUE(result.isOK());
        std::string config = result.getValue();
        ...
        return stdx::make_unique<NarkDbRecordStore>(
            &txn, ns, uri, kNarkDbEngineName, true, false, cappedMaxSize, cappedMaxDocs);
    }

    RecoveryUnit* newRecoveryUnit() final {
        return new NarkDbRecoveryUnit(_sessionCache);
    }
...
std::unique_ptr<HarnessHelper> newHarnessHelper() {
    return stdx::make_unique<NarkDbHarnessHelper>();
}

TEST(NarkDbRecordStoreTest, GenerateCreateStringEmptyDocument) {
    BSONObj spec = fromjson("{}");
    StatusWith<std::string> result = NarkDbRecordStore::parseOptionsField(spec);
    ASSERT_OK(result.getStatus());
    ASSERT_EQ(result.getValue(), "");  // "," would also be valid.
}

TEST(NarkDbRecordStoreTest, GenerateCreateStringUnknownField) {
...
TEST(NarkDbRecordStoreTest, GenerateCreateStringNonStringConfig) {
...
TEST(NarkDbRecordStoreTest, GenerateCreateStringEmptyConfigString) {
...
TEST(NarkDbRecordStoreTest, GenerateCreateStringInvalidConfigStringOption) {
    BSONObj spec = fromjson("{configString: 'abc=def'}");
    ASSERT_EQ(NarkDbRecordStore::parseOptionsField(spec), ErrorCodes::BadValue);
}

TEST(NarkDbRecordStoreTest, GenerateCreateStringValidConfigStringOption) {
    BSONObj spec = fromjson("{configString: 'prefix_compression=true'}");
    ASSERT_EQ(NarkDbRecordStore::parseOptionsField(spec),
              std::string("prefix_compression=true,"));
}

TEST(NarkDbRecordStoreTest, Isolation1) {
...
TEST(NarkDbRecordStoreTest, Isolation2) {
    unique_ptr<HarnessHelper> harnessHelper(newHarnessHelper());
    unique_ptr<RecordStore> rs(harnessHelper->newNonCappedRecordStore());
...
TEST(NarkDbRecordStoreTest, SizeStorer1) {
    unique_ptr<NarkDbHarnessHelper> harnessHelper(new NarkDbHarnessHelper());
    unique_ptr<RecordStore> rs(harnessHelper->newNonCappedRecordStore());

    string uri = checked_cast<NarkDbRecordStore*>(rs.get())->getURI();

    string indexUri = "table:myindex";
    NarkDbSizeStorer ss(harnessHelper->conn(), indexUri);
    checked_cast<NarkDbRecordStore*>(rs.get())->setSizeStorer(&ss);
    ...
    {
        unique_ptr<OperationContext> opCtx(harnessHelper->newOperationContext());
        rs.reset(new NarkDbRecordStore(
            opCtx.get(), "a.b", uri, kNarkDbEngineName, false, false, -1, -1, NULL, &ss));
    }
    ...
    {
        unique_ptr<OperationContext> opCtx(harnessHelper->newOperationContext());
        NarkDbRecoveryUnit* ru = checked_cast<NarkDbRecoveryUnit*>(opCtx->recoveryUnit());

        {
            WriteUnitOfWork uow(opCtx.get());
            NarkDb_SESSION* s = ru->getSession(opCtx.get())->getSession();
            invariantNarkDbOK(s->create(s, indexUri.c_str(), ""));
            uow.commit();
        }
    ...
    {
        unique_ptr<OperationContext> opCtx(harnessHelper->newOperationContext());
        NarkDbSizeStorer ss2(harnessHelper->conn(), indexUri);
        ss2.fillCache();
        long long numRecords;
        long long dataSize;
...
private:
    virtual void setUp() {
        harnessHelper.reset(new NarkDbHarnessHelper());
        sizeStorer.reset(new NarkDbSizeStorer(harnessHelper->conn(), "table:sizeStorer"));
        rs = harnessHelper->newNonCappedRecordStore();
        NarkDbRecordStore* wtrs = checked_cast<NarkDbRecordStore*>(rs.get());
...
protected:
    ...
    std::unique_ptr<NarkDbHarnessHelper> harnessHelper;
    std::unique_ptr<NarkDbSizeStorer> sizeStorer;
...
TEST_F(SizeStorerValidateTest, InvalidSizeStorerAtCreation) {
    ...
    rs.reset(new NarkDbRecordStore(opCtx.get(),
                                       "a.b",
                                       uri,
                                       kNarkDbEngineName,
                                       false,
...
StatusWith<RecordId> insertBSON(unique_ptr<OperationContext>& opCtx,
    ...
    NarkDbRecordStore* wrs = checked_cast<NarkDbRecordStore*>(rs.get());
    invariant(wrs);
...
// TODO make generic
TEST(NarkDbRecordStoreTest, OplogHack) {
    NarkDbHarnessHelper harnessHelper;
...
TEST(NarkDbRecordStoreTest, OplogHackOnNonOplog) {
    NarkDbHarnessHelper harnessHelper;
...
TEST(NarkDbRecordStoreTest, CappedOrder) {
    unique_ptr<NarkDbHarnessHelper> harnessHelper(new NarkDbHarnessHelper());
...
TEST(NarkDbRecordStoreTest, CappedCursorRollover) {
    unique_ptr<NarkDbHarnessHelper> harnessHelper(new NarkDbHarnessHelper());
...
TEST(NarkDbRecordStoreTest, OplogOrder) {
    unique_ptr<NarkDbHarnessHelper> harnessHelper(new NarkDbHarnessHelper());
    unique_ptr<RecordStore> rs(harnessHelper->newCappedRecordStore("local.oplog.foo", 100000, -1));

    {
        const NarkDbRecordStore* wrs = checked_cast<NarkDbRecordStore*>(rs.get());
        ASSERT(wrs->isOplog());
...
TEST(NarkDbRecordStoreTest, StorageSizeStatisticsDisabled) {
    NarkDbHarnessHelper harnessHelper("statistics=(none)");
...
TEST(NarkDbRecordStoreTest, AppendCustomStatsMetadata) {
    NarkDbHarnessHelper harnessHelper;
    unique_ptr<RecordStore> rs(harnessHelper.newNonCappedRecordStore("a.b"));
    ...
    BSONElement narkDbElement = customStats.getField(kNarkDbEngineName);
    ASSERT_TRUE(narkDbElement.isABSONObj());
    BSONObj narkDb = narkDbElement.Obj();

    BSONElement metadataElement = narkDb.getField("metadata");
    ...
    BSONElement creationStringElement = narkDb.getField("creationString");
    ASSERT_EQUALS(creationStringElement.type(), String);
}

TEST(NarkDbRecordStoreTest, CappedCursorYieldFirst) {
    unique_ptr<NarkDbHarnessHelper> harnessHelper(new NarkDbHarnessHelper());
...
StatusWith<RecordId> insertBSONWithSize(OperationContext* opCtx,
    ...
    NarkDbRecordStore* wtrs = checked_cast<NarkDbRecordStore*>(rs);
    invariant(wtrs);
...
TEST(NarkDbRecordStoreTest, OplogStones_CreateNewStone) {
    NarkDbHarnessHelper harnessHelper;
    ...
    NarkDbRecordStore* wtrs = static_cast<NarkDbRecordStore*>(rs.get());
    NarkDbRecordStore::OplogStones* oplogStones = wtrs->oplogStones();
...
// record is changed.
TEST(NarkDbRecordStoreTest, OplogStones_UpdateRecord) {
    NarkDbHarnessHelper harnessHelper;
...
TEST(NarkDbRecordStoreTest, OplogStones_Truncate) {
    NarkDbHarnessHelper harnessHelper;
...
TEST(NarkDbRecordStoreTest, OplogStones_CappedTruncateAfter) {
    NarkDbHarnessHelper harnessHelper;
...
TEST(NarkDbRecordStoreTest, OplogStones_ReclaimStones) {
    NarkDbHarnessHelper harnessHelper;
...
// 'cappedMaxSize'.
TEST(NarkDbRecordStoreTest, OplogStones_ExceedCappedMaxSize) {
    NarkDbHarnessHelper harnessHelper;
...
TEST(NarkDbRecordStoreTest, OplogStones_AscendingOrder) {
    NarkDbHarnessHelper harnessHelper;
