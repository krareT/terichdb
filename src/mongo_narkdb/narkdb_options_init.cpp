...
MONGO_STARTUP_OPTIONS_VALIDATE(NarkDbOptions)(InitializerContext* context) {
    return Status::OK();
}

MONGO_STARTUP_OPTIONS_STORE(NarkDbOptions)(InitializerContext* context) {
    Status ret = narkDbGlobalOptions.store(moe::startupOptionsParsed, context->args());
    if (!ret.isOK()) {
