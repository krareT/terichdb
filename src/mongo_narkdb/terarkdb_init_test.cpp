...
class NarkDbFactoryTest : public mongo::unittest::Test {
private:
    virtual void setUp() {
        ServiceContext* globalEnv = getGlobalServiceContext();
        ASSERT_TRUE(globalEnv);
        ASSERT_TRUE(getGlobalServiceContext()->isRegisteredStorageEngine(kNarkDbEngineName));
        ...
        while (sfi->more()) {
            ...
            if (currentFactory->getCanonicalName() == kNarkDbEngineName) {
                found = true;
        ...
        ASSERT_TRUE(found);
        _oldOptions = narkDbGlobalOptions;
    }

    virtual void tearDown() {
        narkDbGlobalOptions = _oldOptions;
        factory = NULL;
    }
...
void _testValidateMetadata(const StorageEngine::Factory* factory,
    ...
    narkDbGlobalOptions.directoryForIndexes = directoryForIndexes;
...
TEST_F(NarkDbFactoryTest, ValidateMetadataEmptyOptions) {
...
TEST_F(NarkDbFactoryTest, ValidateMetadataDirectoryPerDB) {
...
TEST_F(NarkDbFactoryTest, ValidateMetadataDirectoryForIndexes) {
    _testValidateMetadata(
...
TEST_F(NarkDbFactoryTest, CreateMetadataOptions) {
