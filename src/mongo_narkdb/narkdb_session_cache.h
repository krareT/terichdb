...
class NarkDbKVEngine;

class NarkDbCachedCursor {
public:
    NarkDbCachedCursor(uint64_t id, uint64_t gen, NarkDb_CURSOR* cursor)
...
public:
    /**
     * Creates a new NarkDb session on the specified connection.
     *
     * @param conn NarkDb connection
     ...
     *          cached.
     */
    NarkDbSession(NarkDb_CONNECTION* conn, int epoch = -1);
    ~NarkDbSession();

    NarkDb_SESSION* getSession() const {
        return _session;
    }

    NarkDb_CURSOR* getCursor(const std::string& uri, uint64_t id, bool forRecordStore);

    void releaseCursor(uint64_t id, NarkDb_CURSOR* cursor);
...
private:
    friend class NarkDbSessionCache;

    // The cursor cache is a list of pairs that contain an ID and cursor
    typedef std::list<NarkDbCachedCursor> CursorCache;

    // Used internally by NarkDbSessionCache
    uint64_t _getEpoch() const {
        return _epoch;
    }

    const uint64_t _epoch;
    NarkDb_SESSION* _session;  // owned
...
/**
 *  This cache implements a shared pool of NarkDb sessions with the goal to amortize the
 *  cost of session creation and destruction over multiple uses.
 */
class NarkDbSessionCache {
public:
    NarkDbSessionCache(NarkDbKVEngine* engine);
    NarkDbSessionCache(NarkDb_CONNECTION* conn);
    ~NarkDbSessionCache();
     ...
     */
    void releaseSession(NarkDbSession* session);
    ...
    NarkDbSnapshotManager& snapshotManager() {
        return _snapshotManager;
    }
    const NarkDbSnapshotManager& snapshotManager() const {
        return _snapshotManager;
    }

private:
    NarkDbKVEngine* _engine;  // not owned, might be NULL
    NarkDb_CONNECTION* _conn;         // not owned
    NarkDbSnapshotManager _snapshotManager;
    ...
    typedef std::vector<NarkDbSession*> SessionCache;
