...
class NarkDbSnapshotManager final : public SnapshotManager {
    MONGO_DISALLOW_COPYING(NarkDbSnapshotManager);

public:
    explicit NarkDbSnapshotManager(NarkDb_CONNECTION* conn) {
        invariantNarkDbOK(conn->open_session(conn, NULL, NULL, &_session));
    }

    ~NarkDbSnapshotManager() {
        shutdown();
    }
    ...
    //
    // NarkDb-specific methods
    //

    /**
     * Prepares for a shutdown of the NarkDb_CONNECTION.
     */
    void shutdown();
     ...
     */
    SnapshotName beginTransactionOnCommittedSnapshot(NarkDb_SESSION* session) const;
...
private:
    ...
    NarkDb_SESSION* _session;  // only used for dropping snapshots.
};
