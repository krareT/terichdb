...
/**
 * converts narkdb return codes to mongodb statuses.
 */
inline Status narkDbRCToStatus(int retCode, const char* prefix = NULL) {
    if (MONGO_likely(retCode == 0))
        return Status::OK();

    return narkDbRCToStatus_slow(retCode, prefix);
}

#define invariantNarkDbOK(expression)                                                       \
    do {                                                                                \
        int _invariantNarkDbOK_retCode = expression;                                        \
        if (MONGO_unlikely(_invariantNarkDbOK_retCode != 0)) {                              \
            invariantOKFailed(                                                          \
                #expression, narkDbRCToStatus(_invariantNarkDbOK_retCode), __FILE__, __LINE__); \
        }                                                                               \
    } while (false)

struct NarkDbItem : public NarkDb_ITEM {
    NarkDbItem(const void* d, size_t s) {
        data = d;
        size = s;
    }
    NarkDbItem(const std::string& str) {
        data = str.c_str();
        size = str.size();
    }
    ...
    const NarkDb_ITEM* Get() const {
        return this;
    }
};

class NarkDbUtil {
    MONGO_DISALLOW_COPYING(NarkDbUtil);
...
public:
     ...
     */
    static Status exportTableToBSON(NarkDb_SESSION* s,
     ...
     * List of statistics keys NarkDb_STAT_* can be found in narkdb.h.
     */
    static StatusWith<uint64_t> getStatisticsValue(NarkDb_SESSION* session,
                                                   const std::string& uri,
                                                   const std::string& config,
                                                   int statisticsKey);
    ...
    static StatusWith<ResultType> getStatisticsValueAs(NarkDb_SESSION* session,
                                                       const std::string& uri,
                                                       const std::string& config,
                                                       int statisticsKey);
    ...
    static int64_t getIdentSize(NarkDb_SESSION* s, const std::string& uri);

    /**
     * Returns a NarkDb_EVENT_HANDER with MongoDB's default handlers.
     ...
     * destructor.
     */
    static NarkDb_EVENT_HANDLER defaultEventHandlers();

    class ErrorAccumulator : public NarkDb_EVENT_HANDLER {
    public:
        ErrorAccumulator(std::vector<std::string>* errors);

    private:
        static int onError(NarkDb_EVENT_HANDLER* handler,
                           NarkDb_SESSION* session,
                           int error,
                           const char* message);

        using ErrorHandler = int (*)(NarkDb_EVENT_HANDLER*, NarkDb_SESSION*, int, const char*);
    ...
    /**
     * Calls NarkDb_SESSION::validate() on a side-session to ensure that your current transaction
...
class NarkDbConfigParser {
    MONGO_DISALLOW_COPYING(NarkDbConfigParser);

public:
    NarkDbConfigParser(StringData config) {
        invariantNarkDbOK(
            narkdb_config_parser_open(NULL, config.rawData(), config.size(), &_parser));
    }

    NarkDbConfigParser(const NarkDb_CONFIG_ITEM& nested) {
        invariant(nested.type == NarkDb_CONFIG_ITEM::NarkDb_CONFIG_ITEM_STRUCT);
        invariantNarkDbOK(narkdb_config_parser_open(NULL, nested.str, nested.len, &_parser));
    }

    ~NarkDbConfigParser() {
        invariantNarkDbOK(_parser->close(_parser));
    }

    int next(NarkDb_CONFIG_ITEM* key, NarkDb_CONFIG_ITEM* value) {
        return _parser->next(_parser, key, value);
    }

    int get(const char* key, NarkDb_CONFIG_ITEM* value) {
        return _parser->get(_parser, key, value);
    }

private:
    NarkDb_CONFIG_PARSER* _parser;
};

// static
template <typename ResultType>
StatusWith<ResultType> NarkDbUtil::getStatisticsValueAs(NarkDb_SESSION* session,
                                                            const std::string& uri,
                                                            const std::string& config,
                                                            int statisticsKey) {
...
// static
template <typename ResultType>
ResultType NarkDbUtil::_castStatisticsValue(uint64_t statisticsValue) {
