...
class NarkDbRecordStoreThread : public BackgroundJob {
public:
    NarkDbRecordStoreThread(const NamespaceString& ns)
        : BackgroundJob(true /* deleteSelf */), _ns(ns) {
        _name = std::string("NarkDb RecordStoreThread: ") + _ns.toString();
    }
    ...
    bool _deleteExcessDocuments() {
        ...
        checked_cast<NarkDbRecoveryUnit*>(txn.recoveryUnit())->markNoTicketRequired();
        ...
        try {
            ...
            NarkDbRecordStore* rs =
                checked_cast<NarkDbRecordStore*>(collection->getRecordStore());
            ...
            severe() << "error in NarkDbRecordStoreThread: " << e.what();
            fassertFailedNoTrace(!"error in NarkDbRecordStoreThread");
        } catch (...) {
            fassertFailedNoTrace(!"unknown error in NarkDbRecordStoreThread");
        }
        return true;
    }
...
bool NarkDbKVEngine::initRsOplogBackgroundThread(StringData ns) {
    ...
    if (storageGlobalParams.repair) {
        LOG(1) << "not starting NarkDbRecordStoreThread for " << ns
    ...
    if (_backgroundThreadNamespaces.count(nss)) {
        log() << "NarkDbRecordStoreThread " << ns << " already started";
    } else {
        log() << "Starting NarkDbRecordStoreThread " << ns;
        BackgroundJob* backgroundThread = new NarkDbRecordStoreThread(nss);
        backgroundThread->go();
