...
//     PrimaryKey to RecID mapping；但这样就不能一个 DataIndex 配多个 BlobStore，
//     虽然可以通过 LazyUnionDFA 实现与 One Index Many Store 相近的性能，这必须先将
//     整个数据集排序，再按顺序分区创建多个 NestTrieDAWG
//   * 结论：PrimaryKey 仍然使用单独的 Index，一个 PrimaryIndex 对应多个 BlobStore
//          BlobStore 中如果不保存 PrimaryKey，就需要从 PrimaryIndex 中根据 RecID
