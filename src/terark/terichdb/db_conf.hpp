...
#define TERICHDB_NON_COPYABLE_CLASS(Class) \
...
namespace terark { namespace terichdb {
	...
	class TERICHDB_DLL Schema;
	typedef boost::intrusive_ptr<Schema> SchemaPtr;

	struct TERICHDB_DLL ColumnMeta {
		uint32_t fixedLen;
	...
	//	ColumnMeta();
#if defined(TERICHDB_SCHEMA_COMPILER)
//		SchemaPtr realtype;
	...
	class TERICHDB_DLL Schema : public RefCounter {
...
//		bool   m_isPrimary : 1;
	...
	*/
	...
	class TERICHDB_DLL SchemaSet : public RefCounter {
		struct TERICHDB_DLL Hash {
			size_t operator()(const SchemaPtr& x) const;
			size_t operator()(fstring x) const;
		};
		struct TERICHDB_DLL Equal {
	...
	class TERICHDB_DLL SchemaConfig : public RefCounter {
	public:
		struct Colproject {
			uint32_t colgroupId;
