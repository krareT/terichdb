#ifndef __nark_rank_select_hpp__
#define __nark_rank_select_hpp__
...
class NARK_DLL_EXPORT rank_select
...
class NARK_DLL_EXPORT rank_select_se
	: public RankSelectConstants<256>, public febitvec {
public:
	rank_select_se();
...
///
class NARK_DLL_EXPORT rank_select_il : public RankSelectConstants<256> {
protected:
	struct Line {
		uint32_t      rlev1;
...
public:
	...
	void push_back(bool val) {
		if (nark_likely(m_size < m_lines.size() * LineBits))
			this->set(m_size++, val);
		else
	...
	bool operator[](size_t i) const { // alias of 'is1'
		assert(i < m_size);
		return nark_bit_test(m_lines[i/LineBits].words, i%LineBits);
	}
	...
	void set0(size_t i) {
		assert(i < m_size);
		nark_bit_set0(m_lines[i/LineBits].words, i%LineBits);
	}
	void set1(size_t i) {
		assert(i < m_size);
		nark_bit_set1(m_lines[i/LineBits].words, i%LineBits);
	}
	...
	static bool fast_is1(const Line* l, size_t i) { return  nark_bit_test(l[i/LineBits].words, i%LineBits); }
	static bool fast_is0(const Line* l, size_t i) { return !nark_bit_test(l[i/LineBits].words, i%LineBits); }
...
#endif // __nark_rank_select_hpp__
