...
//	assert(m_pos + length > m_end);
	...
	if (nark_unlikely(NULL == m_beg))
	{
		if (nark_unlikely(0 == m_capacity)) // unbuffered
		{
			length = read_min_max(vbuf, length, length);
			if (nark_unlikely(0 == length))
...
{
	byte b;
	if (nark_likely(do_fill_and_read(&b, 1)))
		return b;
	else
...
int InputBuffer::test_eof()
{
	...
	try {
		byte tmp;
		if (nark_likely(do_fill_and_read(&tmp, 1))) {
			// readed 1 byte
...
size_t OutputBufferBase<BaseClass>::do_flush_and_write(const void* vbuf, size_t length)
{
	...
	if (nark_unlikely(length >= m_capacity)) // remain bytes more than capacity, or unbuffered
	{
		flush_buffer();
