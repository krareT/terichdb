...
public:
	...
	{
		if (nark_likely(m_pos+length <= m_end)) {
...
public:
	...
	size_t read(void* vbuf, size_t length)
	{
		if (nark_likely(m_pos+length <= m_end && m_prefetched)) {
			memcpy(vbuf, m_pos, length);
			m_pos += length;
			return length;
		} else
	...
	byte readByte()
	{
		if (nark_likely(m_pos < m_end && m_prefetched))
			return *m_pos++;
		else
			return fill_and_read_byte();
	}
...
#endif // __nark_io_StreamBuffer_h__
