...
void len_distribute()
{
	...
	do {
		...
		if (nark_unlikely(nil == lenlink[k].head))
			lenlink[k].head = p;
		else
...
void distribute(ptrdiff_t first, ptrdiff_t kpos, ptrdiff_t cnt)
{
	...
	if (codefun) {
		do {
			...
			assert(k < radix);
			if (nark_unlikely(nil == keylink[k].head))
				keylink[k].head = p;
			else
...
void sort()
{
	if (nark_unlikely(0 == max_key_len))
		// not need sort
		return;
