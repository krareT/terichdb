...
#include "stdafx.h"
#include <mongo_terichdb/record_codec.h>
...
int main(int argc, char* argv[])
{
	...
	using namespace mongo::terichdb;
	terark::LineBuf filebuf;
	const char* schemaFile = argv[1];
	const char* bsonFile   = argv[2];
	terark::Auto_fclose fp(fopen(bsonFile, "rb"));
	if (!fp) {
		fprintf(stderr, "fopen(%s, rb) = %s\n", bsonFile, strerror(errno));
		return 2;
	}
	valvec<char> recBuf;
	terark::terichdb::SchemaConfig schema;
	schema.loadJsonFile(schemaFile);
	mongo::terichdb::SchemaRecordCoder coder;
	mongo::terichdb::SchemaRecordCoder::FieldsMap fields2;
	filebuf.read_all(fp);
	printf("file.len=%zd\n", filebuf.size());
